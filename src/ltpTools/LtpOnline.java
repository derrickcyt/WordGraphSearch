...
public class LtpOnline {
	...
	public static Map<String, LTPDATA> doltp(String title) throws IOException {
		...
		// }
		// innet.close();
			List<LTPDATA> ltpdatas = LtpService.getrParser().doLtp(title);
			for (LTPDATA ltpdata : ltpdatas) {
				map.put(ltpdata.getId(), ltpdata);
			}
	...
	public static String ltpAnalysis(Map<String, LTPDATA> map) {
		for (String ss : map.keySet()) {
			if (map.get(ss).content.equals("就")
					&& map.get(ss).relate.equals("ADV"))
				return "false";
		}
		String rootid = "";
		String result = "";
	...
	public static String findltpbegin(String str, String end, String parent,
			Map<String, LTPDATA> map) {
		...
		while (i > -1) {
			...
			if (map.get(si).parent.equals(parent)
					&& (map.get(si).relate.equals("SBV"))) {
				// if(map.get(si).content.equals("就"))return str;
	...
	public static String findltpAfter(String str, String begin, String parent,
			Map<String, LTPDATA> map) {
		...
		while (map.containsKey(String.valueOf(i))) {
			...
			if (map.get(si).parent.equals(parent)
					&& (map.get(si).relate.equals("VOB") || map.get(si).relate
							.equals("COO"))) {
	...
	public static String getAtt(Map<String, LTPDATA> map, String id) {
		...
		for (int i = 0; i < iid; i++) {
			if (map.get(String.valueOf(i)).parent.equals(id)
					&& (map.get(String.valueOf(i)).relate.equals("ATT") || map
							.get(String.valueOf(i)).relate.equals("POB"))) {
				if (map.get(String.valueOf(i)).pos.equals("q")) {
					if ((i - 1) > -1) {
						if (map.get(String.valueOf(i - 1)).pos.equals("m")
								&& map.get(String.valueOf(i - 1)).relate
										.equals("ATT"))
							result = result
									+ map.get(String.valueOf(i - 1)).content
									+ map.get(String.valueOf(i)).content;
					}
				} else
