...
import similarity.BlankUtils;
import similarity.WordSimilarity;
import similarity.HownetMeta;
import similarity.BaseSememeParser;

import com.google.common.collect.HashMultimap;
import com.google.common.collect.Multimap;

/**
 * 概念解析�? 包括概念的加载，内部组织、索引�?查询以及概念的相似度计算�?
 * 概念保存到数组中, 没有保存到Map�? 以尽量降低对内存空间的使�?
 * 算法的核心�?想请参看论文《汉语词语语义相似度计算研究�?
 * <br/><br/>
 * improvement:
 * <ol>
 * 	<li>两个义原集合的运算方式，支持均�?方式或Fuzzy方式</li>
 * </ol>
 * 
...
public abstract class BaseConceptParser implements HownetMeta, WordSimilarity{
	...
	/**
	 * 从XML格式文件输入流中加载概念知识。用户自定义的领域概念，也可以�?过该方式加载到词典中
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * 获取两个词语的相似度，如果一个词语对应多个概念，则返回相似度�?��的一�?
	 * 
	 * @param word1
	 * @param word2
	 * @see ke.commons.similarity.Similariable
	...
	/**
	 * 根据名称获取对应的概念定义信息，由于�?��词语可能对应多个概念，因此返回一个集�?
	 * 
	 * @param key 要查找的概念名称
	 * @return
	 */
	...
	/**
	 * 获取两个概念之间的相似度，如果两个概念的词�?都不相同，则直接返回0;
	 * 否则，计算对应义元的相似度，并加权求�?
	 * 
	 * @param c1 第一个参与运算的概念
	 * @param c2 第二个参与运算的概念
	...
	public double getSimilarity(Concept c1, Concept c2) {
		...
		// 如果概念1或�?概念2有一个为空，或�?两个概念的词性不同，直接返回0
	...
	/**
	 * 计算两个义原集合的相似度，每�?��集合都是�?��概念的某�?��义原集合，如第二基本义原、符号义原�?关系义原等，
	 * 可以采用多种方式计算两个义原集合的相似度，如均�?法�?Fuzzy运算�?
	 * 
	 * @param set1 义原集合1
	 * @param set2 义原集合2
	 * @return
	...
	/**
	 * 考虑到义原集合中的义原先后关系影响不大，因此计算两个集合的义原相似度的平均�?作为这两个义原集合的相似�? 
	 * 即此处采用的是均值方法：
